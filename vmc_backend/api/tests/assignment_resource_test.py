...
class AssignmentResourceTest(ResourceTestCase):
    ...
    def test_get_list_json(self):
        ...
        ids_from_response = set([deserialized_resp['objects'][0]['id'],
                                 deserialized_resp['objects'][1]['id']])
        ids_from_db = set([self.as1.pk, self.as2.pk])
        self.assertEqual(ids_from_response, ids_from_db, "The set of IDs "
                         "returned should be the same given that we have no "
                         "consistent ordering constraints")
